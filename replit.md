...
## Recent Updates
- **Event Privacy & Suspension Separation (2025-10-13)**: Separated event privacy (owner-controlled) from suspension (admin-only). Removed Suspend Event checkbox from event form, ensuring only admins can suspend via detail page button. Owners cannot override admin suspensions. Removed automatic linking between ticketPurchasesEnabled and isPrivate for independent control.
- **Registry PII Removal (2025-09-23)**: Removed all personally identifiable information (PII) from registry_records table including recipient names, emails, purchaser emails, and IP addresses. Registry now stores only essential visual data, event details, and user IDs while preserving the exact visual presentation of collectible pages.
- **Digital Collectibles Terminology (2025-09-23)**: Refactored language from "Registry Records" to "Digital Collectibles" for better user understanding. Users create digital collectibles (12/15 tickets) from validated tickets, with optional NFT minting as a blockchain upgrade feature. Updated all UI references: "Registry" → "Collectibles", clearer value propositions, and simplified language throughout.
- **Simplified NFT Minting (2025-09-23)**: Redesigned NFT minting as an optional "permanent save" feature on collectible pages. Collectible creation (12/15 tickets) remains the main revenue source. Users can optionally mint NFTs directly from collectible pages, paying gas fees (~$0.01-$0.50 on Base L2) from their wallets. Removed complex monitoring sessions in favor of direct blockchain interaction.
