...
export function getExplorerUrl(txHash: string, paymentMethod: 'Bitcoin' | 'Ethereum' | 'USDC'): string {
  switch (paymentMethod) {
    case 'Bitcoin':
...
export function formatTransactionData(
  ...
  transactions.forEach((tx, index) => {
    ...
    output += `  Hash: ${tx.hash}\n`;
    output += `  From: ${tx.from}\n`;
    output += `  Amount: ${tx.value} ${paymentMethod === 'USDC' ? 'USDC' : paymentMethod === 'Bitcoin' ? 'BTC' : 'ETH'}\n`;
